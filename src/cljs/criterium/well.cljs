...
(ns criterium.well
  (:use-macros [criterium.cljs-macros
                :only [unsign limit-bits mat0-pos mat0-neg add-mod-32]]))

(defn bit-shift-right-ns [a b]
  (bit-shift-right-zero-fill a b))
...
http://www.iro.umontreal.ca/~panneton/WELLRNG.html"
  ([] (well-rng-1024a
       (long-array 32 (take 32 (repeatedly #(rand-int max-int-val))))
       (rand-int 32)))
