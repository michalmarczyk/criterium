...
(defn zignor-init
  ...
  (let [c (int c)
    ...
    (loop [i (int 2)
           f f]
      (aset s-adzigx i
            (js/Math.sqrt (* -2e0 (js/Math.log (+ (/ v (aget s-adzigx (dec i))) f)))))
      (when (< i c)
        (recur
         (inc i)
         (js/Math.exp (* -0.5e0 (aget s-adzigx i) (aget s-adzigx i))))))
  ...
  ([rng-seq [^doubles s-adzigr ^doubles s-adzigx zignor-r mask]]
     (letfn [(random-normal-tail
               ...
               (loop [rng-seq rng-seq]
                 (let [x (/ (js/Math.log (first rng-seq)) min)
                       y (js/Math.log (first (next rng-seq)))]
                   (if (>= (* -2e0 y) (* x x))
                     (if negative
       ...
       (let [[deviate rng-seq]
             (loop [rng-seq rng-seq]
               (let [r  (first rng-seq)
                     u  (double (- (* 2e0 r) 1e0))
                     i  (bit-and
                         (int (* max-int-val (first (drop 1 rng-seq))))
                         mask)]
                 ...
                 (if (< (Math/abs u) (nth s-adzigr i))
                   ...
                   (if (zero? i)
                     ...
                     (let [x (* u (nth s-adzigx i))
                           f0 (js/Math.exp
                               (* -0.5e0
                                  (- (js/Math.pow (nth s-adzigx i) 2) (sqr x))))
                           f1 (js/Math.exp
                               (* -0.5e0
                                  (- (js/Math.pow (nth s-adzigx (inc i)) 2)
                                     (sqr x))))]
