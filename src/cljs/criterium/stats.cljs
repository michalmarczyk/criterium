...
(defn confidence-interval
  ...
  (let [n-sigma 1.96                    ; use 95% confidence interval
        delta (* n-sigma (js/Math.sqrt variance))]
...
(defn erf
  ...
  (let [x (double x)
        ...
        p 0.3275911
        t (/ (+ 1.0 (* p x)))
        value (- 1.0 (* (polynomial-value t a) (js/Math.exp (- (* x x)))))]
    (* sign value)))
...
(defn normal-cdf
  ...
  [x]
  (* 0.5 (+ 1.0 (erf (/ x (js/Math.sqrt 2.0))))))
...
(defn modal-estimation-constant
  ...
  [h-k sample-variance]
  (js/Math.sqrt (+ 1 (/ (sqr h-k) sample-variance))))
...
(defn gaussian-weight
  ...
  [t]
  (let [k (js/Math.pow (* 2 js/Math.PI) -0.5)]
    (* k (js/Math.exp (/ (* t t) -2)))))
